...
import com.epam.digital.data.platform.notification.dto.client.NotificationTemplateAttributeDto;
import com.epam.digital.data.platform.notification.dto.client.SaveNotificationTemplateInputDto;
...
class InboxNotificationLoaderTest {
  ...
  protected static File getFile(String path) throws URISyntaxException {
    ...
    URL resourceUrl = InboxNotificationLoaderTest.class.getResource(path);
    if (resourceUrl == null) {
      throw new IllegalArgumentException("Resource not found in classpath: " + path);
    }

    return Path.of(resourceUrl.toURI()).normalize().toFile();
  }
}